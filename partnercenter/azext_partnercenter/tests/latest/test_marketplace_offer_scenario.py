...
class PartnerCenterMarketplaceOfferScenarioTest(ScenarioTest):
    ...
    def _create_offer(self):
        self.cmd(
            "partnercenter marketplace offer create --id {offer_id} -a {offer_alias} --type AzureContainer",
            checks=[self.check("id", "{offer_id}"), self.check("alias", "{offer_alias}")],
        )

    def _show_offer(self):
        self.cmd(
            "partnercenter marketplace offer show --id {offer_id}",
            checks=[
    ...
    def _list_offers_with_created_offer_in_result(self):
        self.cmd(
            "partnercenter marketplace offer list", checks=[self.check("[?id=='{offer_id}'].id | [0]", "{offer_id}")]
        )
