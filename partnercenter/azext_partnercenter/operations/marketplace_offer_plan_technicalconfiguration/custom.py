...
def add_technical_configuration_bundle(client, offer_id, plan_id, cluster_extension_type=None, tenant_id=None,
                                       ...
                                       repository_name=None, tag=None, digest=None, package_path=None):
    print('config bundle hit')
    print('cluster extension type: ' + cluster_extension_type)
    ...
    if hasattr(technical_configuration_bundle, 'cluster_extension_type'):
        print('there is no cluster_extension_type attribute')
        ...
        if _has_existing_package(technical_configuration_bundle):
            print("package_references exists and has a length greater than zero")
        else:
            print("package_references does not exist or has a length of zero")
            print(f"package_path is {package_path}")
            result = client.add_managed_app_bundle(offer_id, plan_id, package_path)

        # check if package references is empty
        # if empty call post package
        # get fileSasUri from post package result
        # extend params to take app_zip from location on disk and upload it to fileSasUri
        # call put on /{productid}/packages/{packageid} with a state of Uploaded - need an If-Match header on the @odata.etag
        # call put on package configuration with a package reference and a deploymentMode of 'incremental' - need an If-Match header on the @odata.etag

        # things to check
        #     1. how does a non-empty package reference work
