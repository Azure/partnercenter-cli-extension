...
def load_command_table(commands_loader, _):
    ...
    with commands_loader.command_group('partnercenter marketplace offer listing', command_type, custom_command_type=custom_command_type, is_preview=True) as g:
        g.custom_show_command('show', 'get_listing', table_transformer=None)
        g.custom_command('delete', 'delete_listing', confirmation=True)
        g.generic_update_command('update', custom_func_name='update_listing', setter_arg_name='listing_model')
