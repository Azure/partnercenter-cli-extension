...
def validate_test_drive(cmd, namespace):
    ...
    if namespace.test_drive and (offer.type != OfferType.AZURETHIRDPARTYVIRTUALMACHINE and offer.type != OfferType.AZUREAPPLICATION):
        raise CLIError(f'Test Drive can only be enabled for the following offer types: {OfferType.AZURETHIRDPARTYVIRTUALMACHINE}, {OfferType.AZUREAPPLICATION}')
